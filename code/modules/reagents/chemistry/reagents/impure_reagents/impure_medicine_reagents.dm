...
/datum/reagent/inverse/penthrite/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	for(var/datum/wound/iter_wound as anything in affected_mob.all_wounds)
		iter_wound.adjust_blood_flow(1-creation_purity * REM * seconds_per_tick)
	var/need_mob_update
	need_mob_update = affected_mob.adjustBruteLoss(5 * (1-creation_purity) * REM * seconds_per_tick, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjustOrganLoss(ORGAN_SLOT_HEART, (1 + (1-creation_purity)) * REM * seconds_per_tick, required_organ_flag = affected_organ_flags)
...
/datum/reagent/inverse/space_drugs
	name = "Naloxone"
...
/datum/reagent/inverse/space_drugs/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
...
/datum/reagent/inverse/aranesp/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	switch(current_cycle)
		...
		if(46 to INFINITY)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_warning("It feels like your head is going to implode in itself!!"))
				affected_mob.adjust_eye_blur(10 SECONDS * REM * seconds_per_tick)
...
/datum/reagent/inverse/happiness/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume)//Splashing people with plasma is stronger than fuel!
	. = ..()
...
/datum/reagent/inverse/baldium/on_mob_end_metabolize(mob/living/carbon/affected_mob)
	. = ..()
	affected_mob.remove_atom_colour(ADMIN_COLOUR_PRIORITY, "#fe0000")
...
/datum/reagent/inverse/baldium/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	switch(current_cycle)
		if(5)
			affected_mob.add_atom_colour("#fe0000", ADMIN_COLOUR_PRIORITY)
